...
class UpperLevel(nn.Module):
    def __init__(self, obs_space, image_dim, memory_dim, final_instr_dim, embedding_layer):
        ...
        # attn unit
        self.attention = ScaledDotProductAttention(temperature=np.power(self.final_instr_dim, 0.5))
        ...
        self.instr_historysg_attn = nn.ModuleList([
            REFER(d_model=self.final_instr_dim, d_inner=self.final_instr_dim, n_head=2, d_k=self.final_instr_dim//2, d_v=self.final_instr_dim//2, dropout=0.2)
            for _ in range(2)])
        ...
        # decoder
        self.decoder = Decoder(obs_space["instr"], self.final_instr_dim, embedding_layer, self.memory_dim, numLayers=self.numLayers)
        self.MAX_SEQ_LEN = 15
    ...
    def forwardDecode(self, visual_embedding, memory, 
    overall_goal_embedding, overall_goal,
    subgoal_histories, beamSize=1):
        ...
        overall_goal_embedding_attn, _ = self.attention(keys, overall_goal_embedding, overall_goal_embedding, mask)
        ...
        sg, logProbs = self.decoder.forwardDecode(hidden, maxSeqLen=self.MAX_SEQ_LEN, inference='greedy', beamSize=beamSize)
        return sg, memory

    def forward(self, visual_embedding, memory, 
    overall_goal_embedding, overall_goal,
    subgoal_histories, 
    teacher_forcing=True,
    gt_subgoal=None, gt_subgoal_embedding=None):
        ...
        if teacher_forcing:
            ...
            sg, logProbs = self.decoder.forwardDecode(hidden, maxSeqLen=self.MAX_SEQ_LEN)
...
class LowerLevel(nn.Module):
    def __init__(self, action_space,
                 image_dim, memory_dim, final_instr_dim):
        super().__init__()
    ...
    instr, instr_embedding):
        ...
        subgoal_embedding, attn = self.attention(keys, subgoal_embedding, subgoal_embedding, mask)
        ...
        keys = self.memory2key_instr(memory).unsqueeze(1)
        instr_embedding, attn = self.attention(keys, instr_embedding, instr_embedding, mask)

        language_embedding = (self.sg_linear(subgoal_embedding) + self.instr_linear(instr_embedding)).squeeze(1)
...
class ThoughCloningModel(nn.Module, babyai.rl.RecurrentACModel):
    def __init__(self, obs_space, action_space,
                 image_dim=128, memory_dim=2048, instr_dim=256):
        super().__init__()
        ...
        self.upper_level_policy = UpperLevel(obs_space, image_dim, memory_dim, self.final_instr_dim, self.obs_embedding.word_embedding)

        # define lower_level_policy

        self.lower_level_policy = LowerLevel(action_space, image_dim, memory_dim, self.final_instr_dim)
    ...
    def train_forward(self, obs, memory, teacher_forcing_ratio, instr_embedding=None):
        ...
        gt_subgoal_embedding = self.obs_embedding.get_language_embedding(gt_subgoal, is_subgoal=True)
        ...
        if teacher_forcing:
            # upper level
            logProbs, subgoal, upper_memory = self.upper_level_policy.forward(visual_embedding, upper_memory, instr_embedding, obs.instr, subgoal_histories,
                                                                    teacher_forcing=teacher_forcing, gt_subgoal=gt_subgoal, gt_subgoal_embedding=gt_subgoal_embedding)
            ...
            next_subgoal_history, _ = self.upper_level_policy.attention(keys, gt_subgoal_embedding, gt_subgoal_embedding, mask)

            # update subgoal_histories
            hidden = (subgoal_histories[:, :self.semi_memory_size], subgoal_histories[:, self.semi_memory_size:])
            hidden = self.upper_level_policy.history_rnn(next_subgoal_history.squeeze(1), hidden)
            subgoal_histories = torch.cat(hidden, dim=1)

            # lower level
            dist, value, lower_memory = self.lower_level_policy.forward(visual_embedding, lower_memory, gt_subgoal, gt_subgoal_embedding, obs.instr, instr_embedding)

            # handle memory
            memory = torch.stack((upper_memory, lower_memory, subgoal_histories), dim=1)

            return {"memory" : memory, "dist" : dist, "logProbs": logProbs, "subgoal": gt_subgoal, "predicted_subgoal": subgoal}
        else:
            logProbs, subgoal, upper_memory = self.upper_level_policy.forward(visual_embedding, upper_memory, instr_embedding, obs.instr, subgoal_histories,
                                                                    teacher_forcing=teacher_forcing, gt_subgoal=gt_subgoal, gt_subgoal_embedding=gt_subgoal_embedding)
            # encode subgoal
            subgoal_embedding = self.obs_embedding.get_language_embedding(subgoal, is_subgoal=True)
            ...
            keys = self.upper_level_policy.memory2key_sg(upper_memory).unsqueeze(1)
            next_subgoal_history, _ = self.upper_level_policy.attention(keys, subgoal_embedding, subgoal_embedding, mask)
            ...
            # lower level
            dist, value, lower_memory = self.lower_level_policy.forward(visual_embedding, lower_memory, subgoal, subgoal_embedding, obs.instr, instr_embedding)
    ...
    def forward(self, obs, memory, instr_embedding=None, beamSize=1):
        ...
        subgoal, upper_memory = self.upper_level_policy.forwardDecode(visual_embedding, upper_memory, instr_embedding, obs.instr, subgoal_histories, beamSize=beamSize)
        ...
        return {"memory": memory, "dist": dist, "subgoal": subgoal, "value": value, 'extra_predictions': dict()}
