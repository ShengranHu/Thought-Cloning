...
class BaseAlgo(ABC):
    ...
    def __init__(self, envs, acmodel, num_frames_per_proc, discount, lr, gae_lambda, entropy_coef,
        ...
        self.memory = torch.zeros(shape[1], 3, self.acmodel.memory_size, device=self.device)
        self.memories = torch.zeros(*shape, 3, self.acmodel.memory_size, device=self.device)
    ...
    def collect_experiences(self):
        ...
        # T x P -> P x T -> (P * T) x 1
        exps.mask = self.masks.transpose(0, 1).reshape(-1, 1, 1)
