...
class ImitationLearning(object):
    def __init__(self, args, ):
        ...
        self.obss_preprocessor = utils.TCObssPreprocessor(args.model, observation_space,
        ...
        if self.acmodel is None:
            if getattr(self.args, 'pretrained_model', None):
                ...
                self.acmodel = ThoughCloningModel(self.obss_preprocessor.obs_space, action_space,
    ...
    def run_epoch_recurrence(self, demos, is_training=False, indices=None):
        ...
        for batch_index in range(len(indices) // batch_size):
            ...
            try:
                ...
                logger.info("batch {}, FPS so far {:.3f}, Accuracy {:.3f}, Policy Loss: {:.3f}, Subgoal Loss: {:.3f}, Final Loss: {:.3f}".format(
                    batch_index, frames / (time.time() - start_time) if frames else 0, _log["accuracy"], _log["policy_loss"], _log["final_sg_loss"], _log["final_loss"]))
    ...
    def run_epoch_recurrence_one_batch(self, batch, is_training=False):
        ...
        mask[inds] = 0
        mask = torch.tensor(mask, device=self.device, dtype=torch.float).reshape(-1,1,1)
        ...
        # Memory to be stored
        memories = torch.zeros([len(flat_batch), 3, self.acmodel.memory_size], device=self.device)
        episode_ids = np.zeros(len(flat_batch))
        memory = torch.zeros([len(batch), 3, self.acmodel.memory_size], device=self.device)
        ...
        while True:
            ...
            with torch.no_grad():
                ...
                new_memory = self.acmodel.train_forward(
                    preprocessed_obs,
                    memory[:len(inds), :, :], teacher_forcing_ratio=teacher_forcing, instr_embedding=instr_embedding[:len(inds)])['memory']
        ...
        for _ in range(self.args.recurrence):
            ...
            with torch.cuda.amp.autocast():
                model_results = self.acmodel.train_forward(
                    preprocessed_obs, memory * mask_step, teacher_forcing_ratio=teacher_forcing,
                ...
                loss = policy_loss - self.args.entropy_coef * entropy + self.args.sg_coef * sg_loss
    ...
    def validate(self, episodes, verbose=True):
        ...
        if getattr(self.args, 'multi_env', None):
            agent = utils.load_agent(self.env[0], model_name=self.args.model, argmax=False, TC=True)
        else:
            agent = utils.load_agent(self.env, model_name=self.args.model, argmax=False, TC=True)
    ...
    def train(self, train_demos, writer, csv_writer, status_path, header, reset_status=False):
        ...
        while status['i'] < getattr(self.args, 'epochs', int(1e9)):
            ...
            logger.info("set teacher_forcing_ratio to {}".format(self.teacher_forcing_ratio))
            ...
            if status['i'] % self.args.log_interval == 0:
                ...
                train_data = [status['i'], status['num_frames'], fps, total_ellapsed_time,
                              log["entropy"], log["policy_loss"], log["final_sg_loss"], log["accuracy"]]

                logger.info(
                    "U {} | F {:06} | FPS {:04.0f} | D {} | H {:.3f} | pL {: .3f} | sL {: .3f} | A {: .3f}".format(*train_data))
            ...
            utils.save_model(self.acmodel, self.args.model)
            if status['i'] == 120:
                utils.save_model(self.acmodel, self.args.model + "_epoch{}".format(status['i']))
                self.obss_preprocessor.vocab.save(utils.get_vocab_path(self.args.model + "_epoch{}".format(status['i'])))
            if torch.cuda.is_available():
                self.acmodel.cuda()
            with open(status_path, 'w') as dst:
                json.dump(status, dst)
