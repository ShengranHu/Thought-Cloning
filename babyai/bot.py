...
class Subgoal:
    ...
    def __init__(self, bot=None, datum=None, reason=None, note=None):
        self.bot = bot
        self.datum = datum
        self.reason = reason
        self.note = note
    ...
    def replan_before_action(self):
        ...
        if (self.fwd_cell.is_locked and not got_the_key):
            ...
            if self.carrying:
                ...
                self.bot.stack.append(PickupSubgoal(self.bot, note='to pick up previous carrying'))
                self.bot.stack.append(GoNextToSubgoal(self.bot, drop_pos_cur, note=self.bot.mission.carrying))

                # Go back to the door and open it
                self.bot.stack.append(OpenSubgoal(self.bot, note=self.note))
                self.bot.stack.append(GoNextToSubgoal(self.bot, self.fwd_cell, note='to open it'))

                # Go to the key and pick it up
                self.bot.stack.append(PickupSubgoal(self.bot, note='to unlock the door'))
                self.bot.stack.append(GoNextToSubgoal(self.bot, key_desc, note='to find a key'))

                # Drop the object being carried
                self.bot.stack.append(DropSubgoal(self.bot, note='temporary to find the key'))
                self.bot.stack.append(GoNextToSubgoal(self.bot, drop_pos_cur, note='to drop it temporary'))
            else:
        ...
        if self.fwd_cell.is_locked and self.reason is None:
            self.bot.stack.pop()
            self.bot.stack.append(OpenSubgoal(self.bot, reason='Unlock', note=self.note))
            return
    ...
    def replan_after_action(self, action_taken):
        if action_taken is None or action_taken == self.actions.toggle:
            ...
            if self.reason == 'Unlock':
                ...
                self.bot.stack.append(DropSubgoal(self.bot, note="after open the door"))
                self.bot.stack.append(GoNextToSubgoal(self.bot, drop_key_pos, note='to drop it'))
        else:
            self._plan_undo_action(action_taken)
    ...
    def replan_before_action(self):
        ...
        if (self.reason == 'Open'
            ...
            if not self.carrying:
                ...
                self.bot.stack.pop()
                self.bot.stack.append(GoNextToSubgoal(self.bot, target_obj, reason='Open', note='to open it'))
                self.bot.stack.append(PickupSubgoal(self.bot, note='to unlock the door'))
                self.bot.stack.append(GoNextToSubgoal(self.bot, key_desc, note='to find a key'))
                return
            ...
            if manhattan_distance(target_pos, self.fwd_pos) == 1:
                ...
                if self.fwd_cell.type == 'door' and self.fwd_cell.is_open:
                    ...
                    self.bot.stack.append(GoNextToSubgoal(
                        self.bot, self.fwd_pos + 2 * self.dir_vec, note='to drop it'))
                    return
        else:
        ...
        if np.array_equal(next_cell, self.fwd_pos):
            if self.fwd_cell:
                ...
                if self.carrying:
                    ...
                    # Take back the object being carried
                    self.bot.stack.append(PickupSubgoal(self.bot, note='resume previous carrying'))
                    self.bot.stack.append(GoNextToSubgoal(self.bot, drop_pos_cur, note=self.bot.mission.carrying))

                    # Pick up the blocking object and drop it
                    self.bot.stack.append(DropSubgoal(self.bot, note='to place blocking object'))
                    self.bot.stack.append(GoNextToSubgoal(self.bot, drop_pos_block, note='to drop it'))
                    self.bot.stack.append(PickupSubgoal(self.bot, note='to remove blocking object'))
                    self.bot.stack.append(GoNextToSubgoal(self.bot, self.fwd_pos, note=self.fwd_cell))

                    # Drop the object being carried
                    self.bot.stack.append(DropSubgoal(self.bot, note='temporary to remove blocking object'))
                    self.bot.stack.append(GoNextToSubgoal(self.bot, drop_pos_cur, note='to drop it temporary'))
                    return
                else:
                    drop_pos = self.bot._find_drop_pos()
                    self.bot.stack.append(DropSubgoal(self.bot, note='to place blocking object'))
                    self.bot.stack.append(GoNextToSubgoal(self.bot, drop_pos, note='to drop it'))
                    self.bot.stack.append(PickupSubgoal(self.bot, note='to remove blocking object'))
                    return
            else:
                return self.actions.forward
...
class ExploreSubgoal(Subgoal):
    def replan_before_action(self):
        ...
        if unseen_pos:
            self.bot.stack.append(GoNextToSubgoal(self.bot, unseen_pos, reason='Explore', note='explore'))
            return None
        ...
        if door_pos:
            ...
            self.bot.stack.pop()
            self.bot.stack.append(OpenSubgoal(self.bot, reason=open_reason, note='to explore'))
            self.bot.stack.append(GoNextToSubgoal(self.bot, door_obj, reason='Open', note='to open it'))
            return
...
class Bot:
    ...
    def __init__(self, mission):
        ...
        # record the executed subgoals
        self.history_subgoals = []
    ...
    def replan(self, action_taken=None):
        ...
        sg_sentance = utils.get_subgoal_sentence(self, suggested_action)

        return {'action': suggested_action, 'sg_sentance': sg_sentance}
    ...
    def _process_instr(self, instr):
        ...
        if isinstance(instr, GoToInstr):
            self.stack.append(GoNextToSubgoal(self, instr.desc, note='to complete GoTo mission'))
            return

        if isinstance(instr, OpenInstr):
            self.stack.append(OpenSubgoal(self, note="to complete open mission"))
            self.stack.append(GoNextToSubgoal(self, instr.desc, reason='Open', note='to open it'))
            return

        if isinstance(instr, PickupInstr):
            # We pick up and immediately drop so
            # that we may carry other objects
            self.stack.append(DropSubgoal(self, note='after pickup mission'))
            self.stack.append(PickupSubgoal(self, note='to complete pickup mission'))
            self.stack.append(GoNextToSubgoal(self, instr.desc, note='to pick it up'))
            return

        if isinstance(instr, PutNextInstr):
            self.stack.append(DropSubgoal(self, note='to complete PutNext mission'))
            self.stack.append(GoNextToSubgoal(self, instr.desc_fixed, reason='PutNext', note='to drop it'))
            self.stack.append(PickupSubgoal(self, note='to complete PutNext mission'))
            self.stack.append(GoNextToSubgoal(self, instr.desc_move, note='to pick it up'))
            return
