...
parser.add_argument("--env", default='BabyAI-BossLevel-v0',
...
parser.add_argument("--valid-episodes", type=int, default=0,
...
parser.add_argument("--log", action="store_true", default=False,
                    help="Whether save the log")     
parser.add_argument("--noise-rate", type=float, default=0.0,
                    help="the ratio to inject noise")           
...
drop_note_options = ['temporary to find the key', 'temporary to remove blocking object', 'to complete PutNext mission']
...
def gen_random_subgoal():
    ...
    if np.random.random() < (2/7):
        #open
        random_sg = "unlock {} door" if np.random.random() < (1/10) else "open {} door"
        random_sg.format(random.choice(color_options))
    elif np.random.random() < (2/7):
        #drop
        random_sg = "drop {} {}".format(random.choice(color_options), random.choice(item_options))
        random_sg += " " + random.choice(drop_note_options)
    elif np.random.random() < (1/7):
        #remove
        random_sg = "remove blocking object {} {}".format(random.choice(color_options), random.choice(item_options))
    else:
        #pickup
        random_sg = "pickup {} {}".format(random.choice(color_options), random.choice(item_options))
...
def generate_demos(n_episodes, valid, seed, shift=0):
    ...
    demos_path = utils.get_demos_path(args.demos, args.env, 'agent', n_episodes, time_stamp=True, jobs=args.jobs)
    demos = []
    ...
    while True:
        ...
        try:
            ...
            if reward > 0 and (args.filter_steps == 0 or len(images) <= args.filter_steps):
                demos.append((mission, blosc.pack_array(np.array(images)), directions, actions, subgoals, current_seed))
                just_crashed = False
                if args.print:
                    print("===============Episode===================")
                    ...
                    for t in range(1, T):
                        ...
                        if last_sg == subgoals[t]:
                            ...
                            if last_sg_end == last_sg_start:
                                print("t={}: Thought \"{}\", Action [{}]".format(last_sg_start, last_sg, ','.join(actions_cache)))
                            else:
                                print("t={}-{}: Thought \"{}\", Action [{}, {}, ... , {}]".format(last_sg_start, last_sg_end, last_sg, actions_cache[0],actions_cache[0], actions_cache[-1]))
                    ...
                    if last_sg_end == last_sg_start:
                        ...
                        print("t={}-{}: Thought \"{}\", Action [{},...,{}]".format(last_sg_start, last_sg_end, last_sg, actions_cache[0], actions_cache[-1]))
...
if args.log:
    fh = logging.FileHandler(os.path.join(utils.storage_dir(), 'logs', 'log'+ time.strftime("_%m-%d-%H-%M-%S", time.localtime()) +'.txt'))
